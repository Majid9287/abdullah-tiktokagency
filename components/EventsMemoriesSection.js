  ...
  return (
    <section id="events" className="py-20 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="mt-12">
          <h3 className="text-2xl font-bold text-pink-800 mb-8 text-center">More Upcoming Events</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {items
              ...
              .map((event, index) => (
                <div
                  key={index}
                  className="group bg-white/80 backdrop-blur-sm rounded-2xl overflow-hidden border border-white/50 shadow-lg hover:shadow-xl transition-all"
                >
                  <div className="relative h-48 w-full overflow-hidden">
                    ...
                    />
                    <div
                      className={`absolute inset-0 bg-gradient-to-br ${event.gradient} opacity-60 mix-blend-overlay`}
                    ></div>

                    {/* Date badge */}
                    <div className="absolute top-4 right-4 bg-white/80 backdrop-blur-sm rounded-lg p-2 shadow-md">
                      <div className="text-center">
                        <span className={`block text-xs font-medium ${event.textColor}`}>
                          {new Date(event.date).toLocaleString("default", { month: "short" })}
                        </span>
                        <span className="block text-lg font-bold text-gray-800">{new Date(event.date).getDate()}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-5">
                    <div className="flex items-center mb-3">
                      <div className={`h-0.5 w-6 bg-gradient-to-r ${event.gradient} mr-2`}></div>
                    ...
                    <button
                      className={`w-full py-2 rounded-lg bg-gradient-to-r ${event.gradient} text-white text-sm font-medium transition-all duration-300 hover:shadow-lg transform group-hover:translate-y-0 translate-y-0`}
                    >
                      {event.cta}
                    </button>
                  </div>
        ...
        <div className="mt-20 text-center">
          <h3 className="text-2xl font-bold text-pink-800 mb-4">Explore Our Past Events</h3>
          <p className="text-pink-900/70 mb-8 max-w-2xl mx-auto">
            Browse through our gallery of memories and see what makes our community special.
          </p>
